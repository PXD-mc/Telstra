...
async def ng(ctx,*,pp =""):
  if pp:
    player = pp.replace(" ", "%20")
    print(str(player))
    try:
        request = urllib.request.Request(f"https://apiv2.nethergames.org/v1/players/{player}", headers={'User-Agent': 'Mozilla/91.0'})
    except:
        await ctx.send(f":red_circle: error!") 
    response = urllib.request.urlopen(request)
    get = json.loads(response.read().decode('UTF-8'))
    if not get:
      await ctx.send(":red_circle: invalid player!")
    else:
      async with aiohttp.ClientSession() as cs:
        async with cs.get(f"https://apiv2.nethergames.org/v1/players/{player}", headers={'User-Agent': 'Mozilla/91.0'}) as resp:
          jsonn = await resp.json()
        playerName = str(get["name"])
        embed=discord.Embed(title=playerName, url=f"https://account.nethergames.org/player/{player}", colour=eclr, description=f">>> {str(get['bio'])}")
        kills = str(get["kills"])
        Deaths = str(get["deaths"])
        Wins = str(get["wins"])
        Loss = str(get["losses"])
        guild = str(get["guild"])
        level = str(get["level"])
        creds = str(get["statusCredits"])
        ranks = str(get["ranks"]).replace(',','\n').replace("'","").replace('[','').replace(']','')
        if len(ranks) == 0:
          ranks = 'No ranks'
        tier = str(get["tier"])
        firstJoin = str(get["firstJoin"])
        firstJoin = datetime.datetime.strptime(f"{firstJoin}", '%Y-%m-%d %H:%M:%S').strftime('%d %b %Y')
        firstJoin = str(time.mktime(datetime.datetime.strptime(firstJoin, "%d %b %Y").timetuple())).replace('.0','')
        lastJoin = str(get["lastJoin"])
        lastJoin = datetime.datetime.strptime(f"{lastJoin}", '%Y-%m-%d %H:%M:%S').strftime('%d %b %Y')
        lastJoin = str(time.mktime(datetime.datetime.strptime(lastJoin, "%d %b %Y").timetuple())).replace('.0','')
        BW = str(get["winsData"]["BW"])
        TB = str(get["winsData"]["TB"])
        SW = str(get["winsData"]["SW"])
        MM = int(get["winsData"]["MM"])
        MS = int(get["winsData"]["MS"])
        SG = int(get["winsData"]["SG"])
        SC = int(get["winsData"]["SC"])
        CQ = str(get["winsData"]["CQ"])
        Duels = str(get["winsData"]["Duels"])
        other = SC+SG+MS+MM;
        embed.set_author(name="Telstra | Player Statsㅤㅤㅤㅤㅤㅤㅤㅤ", url="https://www.google.com/url?sa=t&rct=j&q=&esrc=s&source=web&cd=&cad=rja&uact=8&ved=2ahUKEwirw8yp3rb0AhU54jgGHTI_A_IQwqsBegQIERAB&url=https%3A%2F%2Fwww.youtube.com%2Fwatch%3Fv%3DdQw4w9WgXcQ&usg=AOvVaw0aHtehaphMhOCAkCydRLZU",icon_url=ctx.guild.me.avatar_url)
        embed.add_field(name='Kills',value=kills)
        embed.add_field(name='Deaths',value=Deaths)
        if int(Deaths) == 0:
          Deaths = 1
        embed.add_field(name='KDR', value=round(int(kills)/int(Deaths),2))
        embed.add_field(name='Wins',value=Wins)
        embed.add_field(name='Losses',value=Loss)
        if int(Loss) == 0:
          Loss = 1
        embed.add_field(name='WLR', value=round(int(Wins)/int(Loss),2))
        ...
        embed.add_field(name='Join Times', value='Last Join: <t:{}:R>\nFirst Join: <t:{}:R>'.format(str(get["lastJoined"]),str(get["firstJoined"])),inline=False)
        embed.add_field(name='Wins', value=f' BedWars: **{BW}**\n Duels: **{Duels}**\n The Bridge: **{TB}**\n SkyWars: **{SW}**\n Conquest: **{CQ}**\n Other: **{other}**\n\n**[Click here to invite my NetherGames bot](https://discord.com/api/oauth2/authorize?client_id=851757941336965160&permissions=532579462208&scope=bot%20applications.commands)**',inline=False)
        if str(get["online"]) == "true":
          embed.set_footer(text=f'Online on {(str(get["lastServer"])).replace("---", " : ")}', icon_url="https://media.discordapp.net/attachments/847107107278946354/876500756998524978/unknown.png")
        else:
          embed.set_footer(text=f'Offline', icon_url="https://media.discordapp.net/attachments/847107107278946354/876500710940880966/unknown.png")
        embed.set_thumbnail(url=f"https://apiv2.nethergames.org/v1/players/{player}/avatar")
        await ctx.send(embed=embed)
